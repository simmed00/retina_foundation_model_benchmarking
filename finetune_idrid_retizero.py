...
# 添加 RetiZero-main 到路径
sys.path.insert(0, os.path.join(os.path.dirname(__file__), 'RetiZero-main'))
...
def train(train_loader, val_loader, test_loader, model, optimizer, criterion, args):
    ...
    # 创建保存目录
    save_dir = os.path.join(args.save_model_path, args.net_work)
    os.makedirs(save_dir, exist_ok=True)
    ...
    with open(csv_file, 'w', encoding='utf-8-sig', newline='') as f:
        ...
        writer.writerow(['Model', args.net_work])
...
def main(args):
    """主函数"""
    # 构建模型
    weight_path = args.pretrained_weight
    if not os.path.exists(weight_path):
        raise FileNotFoundError(f"预训练权重文件不存在: {weight_path}")
...
def get_parser():
    """解析命令行参数"""
    parser = argparse.ArgumentParser(description='IDRID 数据集微调')
    
    parser.add_argument("--num_classes", type=int, default=5, help="分类类别数（IDRID 为 5 类）")
    parser.add_argument("--num_workers", type=int, default=4, help="数据加载线程数")
    parser.add_argument("--save_model_path", type=str, default="./checkpoints/idrid", help="模型保存路径")
    parser.add_argument("--net_work", type=str, default="lora", help="模型类型")
    ...
    parser.add_argument("--pretrained_weight", type=str, default="./RetiZero.pth", help="预训练权重路径")
...
if __name__ == "__main__":
    ...
    # 基本使用（使用默认参数）
    python finetune_idrid.py
    
    # 指定参数
    python finetune_idrid.py \
        --pretrained_weight ./RetiZero.pth \
        --data_path "I:/Dataset/IDRID/Disease Grading/1. Original Images" \
        --csv_path "I:/Dataset/IDRID/Disease Grading" \
        --batch_size 32 \
        --lr 5e-4 \
        --num_epochs 100 \
        --save_model_path ./checkpoints/idrid
    """
